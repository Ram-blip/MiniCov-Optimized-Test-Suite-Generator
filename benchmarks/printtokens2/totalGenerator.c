...
int main() {
    ...
    system("gcc -Wno-return-type -fprofile-arcs -ftest-coverage  -g -o printtokens2 printtokens2.c");
    // Read and display each line
    int i = 0;

    float coverageArray[4057];
    int coverageIndex[4057];
    // Store the lines of the file in an array
    char *lines[4057];

    // Same for branch coverage
    float branchCoverageArray[4057];
    int branchCoverageIndex[4057];
    // Store the lines of the file in an array
    char *branchLines[4057];

    while (fgets(line, sizeof(line), fp) != NULL) {
        // Run printtokens2 file using each of these test cases
        
        char string[1000] = "./printtokens2 ";
        strcat(string, line);
        ...
        system("gcov -b printtokens2.c > coverage.txt");
        ...
        system("rm printtokens2.gcda");
        system("rm printtokens2.c.gcov");
    ...
    for (int i = 0; i < 4057; i++) {
        for (int j = i + 1; j < 4057; j++) {
