...
int main() {
    ...
    system("gcc -Wno-return-type -fprofile-arcs -ftest-coverage  -g -o schedule2 schedule2.c");
    // Read and display each line
    int i = 0;

    float coverageArray[2679];
    int coverageIndex[2679];
    // Store the lines of the file in an array
    char *lines[2679];

    // Same for branch coverage
    float branchCoverageArray[2679];
    int branchCoverageIndex[2679];
    // Store the lines of the file in an array
    char *branchLines[2679];

    while (fgets(line, sizeof(line), fp) != NULL) {
        // Run schedule2 file using each of these test cases
        
        char string[1000] = "./schedule2 ";
        strcat(string, line);
        ...
        system("gcov -b schedule2.c > coverage.txt");
        ...
        system("rm schedule2.gcda");
        system("rm schedule2.c.gcov");
    ...
    for (int i = 0; i < 2679; i++) {
        for (int j = i + 1; j < 2679; j++) {
