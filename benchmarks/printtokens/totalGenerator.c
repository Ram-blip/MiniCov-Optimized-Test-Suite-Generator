...
int main() {
    ...
    system("gcc -Wno-return-type -fprofile-arcs -ftest-coverage  -g -o printtokens printtokens.c");
    // Read and display each line
    int i = 0;

    float coverageArray[4072];
    int coverageIndex[4072];
    // Store the lines of the file in an array
    char *lines[4072];

    // Same for branch coverage
    float branchCoverageArray[4072];
    int branchCoverageIndex[4072];
    // Store the lines of the file in an array
    char *branchLines[4072];
    //  Took help from Copilot
    while (fgets(line, sizeof(line), fp) != NULL) {
        // Run printtokens file using each of these test cases
        
        char string[1000] = "./printtokens ";
        strcat(string, line);
        ...
        system("gcov -b printtokens.c > coverage.txt");
