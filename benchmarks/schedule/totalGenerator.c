...
int main() {
    ...
    system("gcc -Wno-return-type -fprofile-arcs -ftest-coverage  -g -o schedule schedule.c");
    // Read and display each line
    int i = 0;

    float coverageArray[2634];
    int coverageIndex[2634];
    // Store the lines of the file in an array
    char *lines[2634];

    // Same for branch coverage
    float branchCoverageArray[2634];
    int branchCoverageIndex[2634];
    // Store the lines of the file in an array
    char *branchLines[2634];

    while (fgets(line, sizeof(line), fp) != NULL) {
        // Run schedule file using each of these test cases
        
        char string[1000] = "./schedule ";
        strcat(string, line);
        ...
        system("gcov -b schedule.c > coverage.txt");
